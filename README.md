...
一个轻量、可自托管的 OpenList / SmartSTRM 视频快刷与整理工具。它把目录中的视频随机生成播放列表，提供适合手机、平板和电脑的竖屏视频流，并支持保留、喜欢、加入待删除、导出清单和最终确认删除。

Vidpick 不转码、不代理视频内容。若挂载 SmartSTRM 生成的目录，服务端会直接读取本地 `.strm` 小文件来浏览目录、建立播放列表并取得已有播放地址，无需递归扫描 OpenList；未配置 SmartSTRM 时则自动回退到 OpenList API。两种方式最终都用 HTTP 302 将浏览器跳转到媒体地址，保持很低的 CPU、内存和服务器带宽占用。

## 功能

- 优先从 SmartSTRM 本地目录树选择入口目录，也可回退到 OpenList
- 可选择是否递归扫描子目录
- 只收集视频，忽略图片和其他文件
- 每次随机生成播放顺序
...
- 自定义可拖动播放时间轴，不显示浏览器原生悬停遮罩和音频控件
- SmartSTRM 本地索引和 OpenList 递归扫描均缓存最近 5 分钟的结果
- 手机和平板支持上下滑动
- 电脑支持滚轮、鼠标拖动和键盘操作
...
flowchart LR
    ...
    B -->|"只读挂载"| C["SmartSTRM .strm 索引"]
    C -->|"已有签名播放地址"| B
    B -->|"仅核对和确认删除"| D["OpenList"]
    B -->|"HTTP 302"| A
    A -->|"Range 请求，直接播放"| E["实际存储源"]
    B -->|"原子写入"| F["本地 JSON 状态"]
```

SmartSTRM 是可选的快速路径。配置后，普通目录浏览、随机列表生成和播放都不调用 OpenList API；只有用户在整理页最终确认删除时，Vidpick 才使用受限 OpenList 账号重新核对并删除原文件。`.strm` 内的查询参数不会写入状态文件、日志或浏览器页面，只在播放跳转时使用。
...
| `SMARTSTRM_ROOT` | 空 | 可选的 `.strm` 索引目录；设置后优先使用 |
| `SMARTSTRM_OPENLIST_BASE` | `/` | `.strm` 链接中 OpenList `/d/` 路径需要移除的账号根前缀 |
| `SMARTSTRM_ALLOWED_HOST` | 空 | `.strm` 播放地址允许的唯一主机名，生产环境强烈建议设置 |

`OPENLIST_TOKEN_FILE` 可包含纯令牌，也可使用 `OPENLIST_TOKEN=...` 格式。

## Docker 部署

仓库提供 [Dockerfile](Dockerfile) 和 [compose.example.yaml](compose.example.yaml)。示例使用主机网络，让容器只监听宿主机回环地址，同时访问同样只绑定回环地址的 OpenList。SmartSTRM 目录以只读方式挂载，Vidpick 不修改生成的 `.strm` 文件。

```bash
cp compose.example.yaml compose.yaml
# 修改示例域名、端口、OpenList 内网地址、STRM 挂载和根目录
